...
(function attachMilhasCalculator(global) {
  ...
  const DEFAULT_CPM = Object.freeze({ avios: 39, latam: 18, smiles: null, azul: null, aegean: 68 });
  ...
  const LATAM_REGIONS = Object.freeze({
    AMS: { label: 'Am. Sul' },
    AMC: { label: 'Am. Central' },
    AMN: { label: 'Am. Norte' },
    EUR: { label: 'Europa' },
    OCE: { label: 'Oceania' },
    OM: { label: 'Oriente Médio' },
    AFR: { label: 'África' },
    ASO: { label: 'Ásia Oriental' },
    ASOC: { label: 'Ásia Ocidental' }
  });
  ...
  const LATAM_AIRPORT_REGION = Object.freeze({
    ...
    // Am. Sul (retorna null — origem Brasil)
    GIG: null, GRU: null, CGH: null, SSA: null, REC: null, FOR: null,
    BSB: null, CNF: null, POA: null, FLN: null, EZE: null, AEP: null,
    SCL: null, LIM: null, BOG: null, UIO: null, MVD: null, VCP: null
  });
  ...
  function extractCashAmount(text) {
    ...
    const brlMatches = Array.from(value.matchAll(/(?:BRL|R\$)\s*([\d.]+,\d{2})/gi));
  ...
  function annotateSeatsAero(document) {
    ...
    root.querySelectorAll('.booking-card').forEach((card) => {
      ...
      if (program && flightDate && (program.program === 'Iberia Plus' || program.program === 'Iberia Club')) {
        ...
        const tag = insertFlag(`Tabela Iberia · distância + temporada · ${season}`);
      ...
      if (program && program.program === 'LATAM Pass' && routeDest) {
        ...
        if (operatingFlight.startsWith('LA')) {
          ...
          const match = SEATS_POINTS_PATTERN.exec(pointsElement.textContent);
          const points = match ? parseQuantity(match[1]) : 0;
          const tableValue = LATAM_REGIONS[region.region]
            ? `${region.label} · ${cabinLabel} · ${points.toLocaleString('pt-BR')} pts`
            : `${region.label} · ${cabinLabel}`;
          const tag = insertFlag(`Estimativa LATAM parceiro: ${tableValue}`);
  ...
  global.MilhasCalculator = Object.freeze({
    isIberiaPeak, getLatamRegion, parseSeatsDate, getActiveCabin, getFlightProgram, getExploreNavBadges, getExploreBadgeKind, getExploreNavPriority, getExploreNavGroup, getAlternativePrograms, isDirectExploreTitle, isQatarFeaturedBusinessRoute, buildLatamSearchUrl, extractTripRoute, setCpms, parseQuantity, extractCashAmount, getLatamEstimate, findMentions, calculateCost, formatCurrency, annotateElement, annotateSeatsAero, annotateSeatsTripCards, annotateSeatsExplore, annotateSeatsExploreNav, annotateQatarFeaturedRoutes, annotateSeatSpy, annotateQatarAirways, annotateLatamAirlines, annotateGoogleFlights, annotateDocument, observeDocument
  });
}(globalThis));
