# Histórico de versões

## 1.0.1 — 2026-07-12

- Altera o CPM padrão de Avios para R$ 55 por milheiro.
- Altera o CPM padrão de LATAM Pass para R$ 25 por milheiro.

## 1.0.0 — 2026-07-12

- Primeira versão preparada para a Chrome Web Store.
- Atualiza a chave Pix do desenvolvedor.
- Adiciona documentação de publicação e privacidade.
- Impede que a conversão LATAM leia suas próprias etiquetas.
- Remove etiquetas duplicadas no site da LATAM.
- Aceita como coparticipação LATAM apenas valores nativos em BRL ou no `aria-label`.
- Remove badges legados de conversores anteriores dentro das tarifas LATAM.
- Calcula o total de milhas + dinheiro no LATAM Pass.
- Torna conservadora a conversão do cartão principal do Seats.aero.
- Melhora links LATAM no Google Voos.

## Versões anteriores

- Integrações com Seats.aero, SeatSpy, Iberia, Qatar Airways e LATAM.
- Configuração independente de CPMs.
- Página Sobre, FAQ, badges e atalhos voltados ao público brasileiro.
